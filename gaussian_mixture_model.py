...
class Random_Grid_Search:
    def __init__(self, s, m, p_list, t_n=500):  # 初始化实例参数
        self.s, self.s_len = np.array(s), len(s)
        self.m = m
        self.k_list = k_list
        self.para_range = [[1e-6, 0.1], [1e-4, 1], [0.1, 1]]  # p, q, c 参数范围
    ...
    def func(self, g, x):
        pk = self.pk_cont[g]
        p, q, c = x  # c为最终累积扩散率
        inst_diff = np.zeros(self.s_len)  # 非累积扩散率
    ...
    def mix_func(self, x):
        diff_cont = np.array([self.func(g, x) for g in range(3)])
    ...
    def optima_search(self, c_n=100, threshold=1e-4):  # 搜索过程
        orig_points = self.gener_orig(self.para_range, or_points=[])  # 生成初始搜索点
        samp = self.sample(self.para_range)
        solution = np.apply_along_axis(self.neg_loglike, axis=0, arr=samp + orig_points)[:c_n]
        for u in range(100):
            par_min, par_max = np.min(solution, axis=0), np.max(solution, axis=0)  # 最小、最小值
            c_range = [[par_min[j + 1], par_max[j + 1]] for j in range(len(self.para_range))]  # 重新定界
            samp = self.sample(c_range)
            temp = np.apply_along_axis(self.neg_loglike, axis=0, arr=samp)  # 生成
            solution = np.sort(np.concatenate((temp, solution)), axis=0)[:c_n]
            r = solution[:, 0]
            v = (r[-1] - r[0]) / r[0]
...
class GMM:  # gaussian mixture model
    ...
    def excep_max(self, threshold=10e-3):
        p_list = np.array([0.25, 0.25, 0.25, 0.25])  # 初始化
        cons = {'type': 'eq', 'fun': lambda x:np.sum(x)-1}
        rgs = Random_Grid_Search(self.s, self.m, p_list, t_n=500)
        flag = 1
        logli_cont = []
        while flag > threshold:
            ...
            logli_cont.append(res1[0])
            params = res1[1:]
            # Maximization
            res2 = minimize(self.neg_loglike, p_list, args=(params,), constraints=cons, method='SLSQP')
            p_list = np.array(res2.x)
            logli_cont.append(self.neg_loglike(p_list, params=params))
            flag = (logli_cont[-1] - logli_cont[-2]) / logli_cont[-1]
        return logli_cont[-1], params, p_list
